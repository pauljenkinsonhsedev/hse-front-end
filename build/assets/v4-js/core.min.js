...
$(document).ready(function () {	
	...
	var cookieBannerCheck = Cookies.get('cookies_preferences_set');
	
	if (cookieBannerCheck) {	
		const decodedCookieBannerCheck = window.atob(cookieBannerCheck);

		if ( decodedCookieBannerCheck == 'true') {
	...
	// Essential includes usage (GA)
	var essential_cookie_consent = {'essential':true,'usage':true,'campaigns':false,'settings':false};
	var approved_all_cookie_consent = {'essential':true,'usage':true,'campaigns':false,'settings':false};
	...
	const encodedEssential = window.btoa(essential); 
	const encodedApproved = window.btoa(approved);
	
	
	var cookieBannerValue = 'true';
	const cookieBannerAccepted = window.btoa(cookieBannerValue);
	
	// Sets essential cookies
	
	var cookieExists = Cookies.get('cookies_policy');
...
$.browser.majorVersion=parseInt($.browser.version.substr(0,1));function fixConsole(a){"undefined"===typeof console&&(console={});"undefined"===typeof console.log&&(console.log=a?function(a){alert(a)}:function(){});"undefined"===typeof console.dir&&(console.dir=a?function(a){alert("DIR: "+a)}:function(){})}fixConsole();function platformDetector(){return"www.hse.gov.uk"!=window.location.hostname?platformPath=scriptPath():platformPath=""}
function scriptPath(){for(var a=document.getElementsByTagName("script"),c=a.length;c--;)if(a[c].src){var b=a[c].src.match(/\/?([^\/]+)\.js$/);if(b&&"core.min.js"==b[1]+".js"){if(0<=a[c].src.indexOf("http://"))return platformPath=a[c].src.replace("/assets/v4-js/core.min.js","");c=a[c].src.split("/");b=c.length;for(x=a=0;x<=b;x++)".."==c[x]&&a++;c=window.location.href.split("/");for(x=0;x<=a;x++)c.pop();return platformPath=c.join("/")}}return!1}platformDetector();
function relativeLink(a){return $.browser.msie&&8>$.browser.majorVersion?(sitePath=window.location.protocol+"//"+window.location.hostname,a.replace(sitePath,"")):a}
(function(a){a.fn.extend({analyticsOverlay:function(c){c=a.extend({genericId:"#gId",specificId:"#sId",noOverlay:"#nId"},c);var b=RegExp("[^a-zA-Z0-9]","g");a("body").attr("si:pageID",document.URL.replace("index.htm","").replace(b,"").slice(-32));a("body").prop("si:pageID",document.URL.replace("index.htm","").replace(b,"").slice(-32));return this.each(function(){var e=c,d=a(this);if(a(d).parents(e.genericId).length)var l=0,g="",f=255;else a(d).parents(e.specificId).length?(l=0,g=a(d).parents(e.specificId).attr("id"),
f="navSecondary"==g?240:255-g.length):a(d).parents(e.noOverlay).length?(l=1,f=g=""):(console.log("WARNING: Link is not being tracked in overlay and it is NOT in the noOverlay exclusion list!"),console.log("Link text: "+a(d).text()));var e=a(d)[0].href,h=a(d).text(),k=(g+e+h).replace(b,"").slice(-f);a(d).each(function(c){0===l&&null==a(d).attr("id")&&a(d).attr("id",""+k+"")});a(d).click(function(a){"function"==typeof SiTrackLink&&SiTrackLink(this)})})}})})(jQuery);
...
/*	Audio Player 2.0 - http://wpaudioplayer.com/ */
 ...
 * based on - v1.8.20 - 2012-04-30
 * PM edit to:
 		correct: selected and expanded states on toggled closed tabs
		add: multiselectable="true" where role=tablist
...
e.fillSpace};e.proxied||(e.proxied=e.animated);e.proxiedDuration||(e.proxiedDuration=e.duration);e.animated=d.isFunction(e.proxied)?e.proxied(c):e.proxied;e.duration=d.isFunction(e.proxiedDuration)?e.proxiedDuration(c):e.proxiedDuration;f=d.ui.accordion.animations;var m=e.duration,l=e.animated;!l||f[l]||d.easing[l]||(l="slide");f[l]||(f[l]=function(a){this.slide(a,{easing:l,duration:m||700})});f[l](c)}else e.collapsible&&f?a.toggle():(b.hide(),a.show()),h(!0);a.prev().attr({"aria-expanded":"true",
"aria-selected":"true",tabIndex:0}).focus();b.prev().attr({"aria-expanded":"false","aria-selected":"false",tabIndex:-1}).blur()},_completed:function(a){this.running=a?0:--this.running;this.running||(this.options.clearStyle&&this.toShow.add(this.toHide).css({height:"",overflow:""}),this.toHide.removeClass("ui-accordion-content-active"),this.toHide.length&&(this.toHide.parent()[0].className=this.toHide.parent()[0].className),this._trigger("change",null,this.data))}});d.extend(d.ui.accordion,{version:"@VERSION",
